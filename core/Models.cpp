...
FileItem FileItem::fromJson(const nlohmann::json& json)  //
{
    ...
    // sends "modified_at"
    item.modifiedAt =
        json.value("modified_at", "");  // Use "modified_at" as per OpenAPI spec
...
std::string DirectoryListing::toTreeString()  //
{
    ...
    std::function<void(const DirectoryListing&, std::stringstream&, const std::string&, bool)>
        buildTreeString = //
            [&](const DirectoryListing& dir, std::stringstream& ss, const std::string& indent, bool isLast) { //
        ...
        if (lastSlash != std::string::npos && lastSlash + 1 < dirName.length()) {
             ...
             // Handle trailing slash if needed, though paths should be normalized
        ...
        ss << indent; //
        if (!indent.empty() || dir.path != "/") // Add prefix for non-root items or if indent exists
        {
             ss << (isLast ? "└── " : "├── "); //
        }
         ss << (dirName.empty() ? "." : dirName) << (dirName == "/" ? "" : "/") << std::endl; // Ensure trailing slash for dirs


        std::string childIndent = indent; //
         if (!indent.empty() || dir.path != "/") // Adjust child indent correctly based on whether parent had prefix
         {
             childIndent += (isLast ? "    " : "│   "); //
         }
        ...
        auto formatFileSize = [](size_t size) -> std::string { //
            ...
            ss_size << std::fixed << std::setprecision(1) << fileSize << " " << units[unitIndex]; // Use 1 decimal place
            return ss_size.str(); //
        };
