...
public:
    ...
    /**
     * @brief Get directory listing with full tree structure up to specified depth
     * 
     * @param path Root path to start crawling from
     * @param maxDepth Maximum depth to crawl (0 means no recursion, just the root)
     * @return DirectoryListing Full directory tree structure
     */
    DirectoryListing getDirectoryTreeWithDepth(const std::string& path = "/", int maxDepth = 3);
...
private:
     ...
     * @return DirectoryListing Directory listing for the current path with subdirectories
     */
    DirectoryListing buildDirectoryTree(const std::string& currentPath, int currentDepth, int maxDepth);
};
